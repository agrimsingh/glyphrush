...
12. If an OCR adapter is configured and the page routes to `ocr_fallback`, load sidecar text for that page or invoke the configured OCR command with the PDF path and zero-based page index, bounded by `--ocr-timeout-ms`. Use `ocr-check <pdf> --page-index <N> --strict` to preflight the same adapter contract before scanned/hybrid benchmark runs.
