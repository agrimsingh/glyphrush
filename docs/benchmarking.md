...
Add `--pdf <file>` to run each wrapper against one smoke PDF and emit output bytes, stdout SHA-256, line/word counts, empty-output status, stderr previews, stable `error_kind` values for failures, `smoke_success_count`, and `all_smoke_passed`. Add `--pdf <directory>` to smoke every recursively discovered PDF in stable relative-path order and emit per-baseline aggregate document counts plus up to three `smoke.failure_samples` with failed document path, exit status, `error_kind`, stderr preview, and error. Current smoke failure kinds include `timeout`, `missing_dependency`, `execution_failed`, `spawn_failed`, and `invalid_smoke_target`. Add `--strict` for CI or benchmark preflight scripts: the command still writes its JSON report, then exits nonzero if any describe probe failed or if a smoke PDF was supplied and any smoke probe failed. This catches missing wrapper paths, broken comparison metadata, and missing parser dependencies before a long corpus run.
...
glyphrush manifest test/ --category-from-path --coverage-preset glyphrush-v0 > test/corpus.full.json
...
Use `scripts/bench-liteparse.sh` for the repeatable local speed gate: by default it first preflights the baseline wrappers, then runs the optimized release-profile PDFium backend against `test/` with `test/corpus.datasheets.json`, requires the `glyphrush-v0` baselines, requires baseline quality checks, enforces `--require-speedup-claim liteparse=2.0`, and also enforces `--require-speedup-claim liteparse-no-ocr=1.5` so native-text wins are not hidden behind OCR cost. The seed datasheet path uses `baseline-check --pdf <target> --baseline-preset glyphrush-v0 --strict` to catch missing wrappers or file-specific baseline failures before benchmark work. Set `GLYPHRUSH_BENCH_OUTPUT=<path>` to save the benchmark report and automatically follow it with `feature-parity --bench-report <path> --require-speed-evidence`, which verifies the saved report contains passing, quality-backed LiteParse and LiteParse-no-OCR speed claims. Saved-report runs write JSON to stdout or the chosen file and emit external-baseline progress lines such as `glyphrush: external baseline start baseline=liteparse pdf=...` to stderr before invoking each wrapper, so a long LiteParse run is not a silent zero-byte wait. When `GLYPHRUSH_BENCH_OUTPUT` is set, the script also tees benchmark stderr to `<output-without-.json>.progress.log`; set `GLYPHRUSH_BENCH_PROGRESS_LOG=<path>` to choose a different durable progress log. The default `GLYPHRUSH_BENCH_CATEGORY` is `datasheet`; set `GLYPHRUSH_BENCH_CATEGORY=all` to omit `--eval-category` and evaluate every manifest category in a mixed benchmark corpus. Set `GLYPHRUSH_BENCH_MANIFEST=test/corpus.v0.json GLYPHRUSH_BENCH_COVERAGE_PRESET=glyphrush-v0` for the checked-in v0 corpus; the script then defaults the PDF root to `test/v0` so older root-level test PDFs are not included in the speed denominator, switches the baseline preflight to describe-only so external baselines are not asked to parse the whole large corpus twice, and raises the benchmark per-baseline timeout to 900000 ms for large fixtures. Set `GLYPHRUSH_BENCH_PROBE_PDF=<pdf>` when a full v0 run appears stuck on one document: the script skips the benchmark and follow-up feature-parity command, runs only `baseline-check --pdf <pdf>`, defaults that probe timeout to 60000 ms, and writes a baseline-check JSON report plus optional progress log when `GLYPHRUSH_BENCH_OUTPUT=<path>` is set. By default probe mode uses `--baseline-preset glyphrush-v0`; set `GLYPHRUSH_BENCH_PROBE_BASELINE=liteparse`, `liteparse-no-ocr`, `pymupdf`, or `pdfplumber` to isolate one wrapper. Override probe duration with `GLYPHRUSH_BENCH_PROBE_TIMEOUT_MS=<ms>` or the general `GLYPHRUSH_BENCH_BASELINE_TIMEOUT_MS=<ms>` when you need a different bound. Set `GLYPHRUSH_BENCH_PREFLIGHT=smoke` to force the full `baseline-check --pdf <target>` preflight, `GLYPHRUSH_BENCH_PREFLIGHT=describe` for dependency/metadata-only wrapper checks, or `GLYPHRUSH_BENCH_PREFLIGHT=none` when the benchmark's own `--require-baselines` and `--require-baseline-quality` gates are sufficient. Set `GLYPHRUSH_BENCH_COVERAGE_PRESET=glyphrush-v0` when any custom benchmark manifest covers the full v0 corpus classes and the run should fail unless that coverage is present; leave it unset for the current datasheet-only seed corpus. When both `GLYPHRUSH_BENCH_OUTPUT` and `GLYPHRUSH_BENCH_COVERAGE_PRESET` are set, the follow-up parity check also receives `--require-coverage-preset <preset>` so the saved speed report is checked against the same full claim-readiness surface. PDFium benchmark reports may show `worker_count: 1` even when a higher `--jobs` value is requested, because the adapter reuses loaded document handles and currently serializes corpus-level document extraction to avoid concurrent live PDFium handles in one process. Override `GLYPHRUSH_BENCH_PDF_DIR`, `GLYPHRUSH_BENCH_MANIFEST`, `GLYPHRUSH_BENCH_CATEGORY`, `GLYPHRUSH_BENCH_JOBS`, `GLYPHRUSH_BENCH_BACKEND`, `GLYPHRUSH_BENCH_FEATURES`, `GLYPHRUSH_BENCH_LITEPARSE_SPEEDUP`, `GLYPHRUSH_BENCH_LITEPARSE_NO_OCR_SPEEDUP`, `GLYPHRUSH_BENCH_COVERAGE_PRESET`, `GLYPHRUSH_BENCH_PREFLIGHT`, `GLYPHRUSH_BENCH_PROGRESS_LOG`, `GLYPHRUSH_BENCH_PROBE_PDF`, `GLYPHRUSH_BENCH_PROBE_BASELINE`, `GLYPHRUSH_BENCH_PROBE_TIMEOUT_MS`, `GLYPHRUSH_BENCH_BASELINE_TIMEOUT_MS`, or `GLYPHRUSH_BENCH_OUTPUT` when running a different corpus or saving a report. `baseline-check --pdf <directory>` smokes every recursively discovered `.pdf`/`.PDF` in stable relative-path order and reports `smoke_document_count`, aggregate per-baseline document pass/fail counts, bounded failure samples, and per-document smoke results before a long corpus benchmark. `tools/baselines/liteparse-text.sh` uses LiteParse's `lit parse --format text --quiet` path with OCR enabled by default, while `tools/baselines/liteparse-no-ocr-text.sh` runs the same LiteParse text path with `--no-ocr` for native-text-only timing. You can also set `LITEPARSE_NO_OCR=1` on `liteparse-text.sh`, but the explicit no-OCR wrapper is preferred when reports need distinct baseline names.
