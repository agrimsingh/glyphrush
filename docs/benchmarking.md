...
Table checks score rows recovered from table layout blocks. Eval uses a table block's structured `table.rows[].cells[]` payload when present, then falls back to parsing the block text for compatibility with older artifacts. In v0 those blocks can come from pipe/tab text, table-routed whitespace rows including fixed-width rows with blank cells, or table-routed positioned spans that form aligned multi-column rows when `--span-geometry` is enabled.
