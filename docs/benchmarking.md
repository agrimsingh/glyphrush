...
- `speedup_claims`: one entry per `--require-speedup` request. Each entry records the named baseline, required and actual Glyphrush speedup, speed comparability, whether the speed threshold passed, Glyphrush and baseline quality-check/pass status, `quality_backed`, and a conservative `claim_passed` verdict.
...
- `speedup_claims`: corpus-level verdicts for each `--require-speedup` request, computed from the aggregate corpus baseline comparison and corpus quality status.
- `document_count`.
