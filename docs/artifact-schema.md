...
The v0 CLI supports `--ocr-sidecar <dir>` and `--ocr-command <executable>` as adapter seams. For sidecars, page index `0` of `example.pdf` is loaded from `example.p000000.txt`. For commands, Glyphrush invokes the executable only for OCR-routed pages and passes the PDF path plus zero-based page index as arguments. OCR commands are bounded by `--ocr-timeout-ms`, defaulting to `120000`. OCR text is stored in `ocr_spans` with `provenance: "ocr"` and does not overwrite native spans. When OCR is applied for an OCR-routed page, derived layout/text views are built from the OCR text because the native text has already been classified as missing or low-confidence.
