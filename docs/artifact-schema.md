...
- `layout_blocks`: deterministic text-derived blocks. With positioned native spans, v0 can isolate table-routed aligned row runs into table blocks while preserving surrounding text blocks, otherwise groups spans by vertical gaps and assigns each block the union of only the contributing span boxes.
