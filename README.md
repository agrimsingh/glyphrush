...
- `bindings/python` and `bindings/node`: experimental dependency-free wrappers that shell out to the native CLI and return the same JSON artifact.
- `bindings/wasm`: a `wasm-bindgen` wrapper over `glyphrush-core` and the shared `glyphrush-lopdf` extraction crate; `parse_pdf_bytes` returns the same JSON document artifact, verified by a deep-equal parity test against the CLI.

The dependency-light backend extracts native text through `lopdf`, can preserve simple positioned text spans with approximate boxes when explicitly requested, records cheap drawn-image metadata for direct image XObjects, image-backed form XObjects, and detected inline images without copying pixels, follows nested form image transforms for image coverage, routes OCR-required pages to optional sidecar, command, or HTTP OCR adapters, and emits structured artifacts with parser/backend/source size and modified-time metadata. The experimental PDFium backend opens PDFs through PDFium, extracts native page text, can emit PDFium text-segment boxes when `--span-geometry` is requested, records cheap image-object metadata without rendering pixels on the native extraction path, detects ruled-table vector paths, and can render OCR-routed pages to temporary PPM images for command or HTTP adapters; it does not provide built-in OCR. Bundled OCR engines are intentionally outside the default path. MuPDF was evaluated as a backend candidate and rejected: it is AGPL-3.0 licensed while Glyphrush is MIT, and the BSD-licensed PDFium adapter already provides the measured fast path; `backend-check` keeps reporting the rejected adapter slot so the decision stays visible.
...
`scripts/verify.sh` is the shared local/GitHub CI gate. It runs formatting, Python and Node wrapper tests, the full Rust workspace test suite, clippy with warnings denied, strict `glyphrush-v0` baseline-preset metadata preflight, the datasheet eval gate when ignored root-level local PDFs exist under `test/`, and, when `GLYPHRUSH_VERIFY_PDFIUM=1` is set, the PDFium-generated v0 eval gate, the span-geometry layout gate (`test/corpus.v0.layout.json`), the sidecar OCR-applied gate (`test/corpus.v0.ocr.json` with `test/ocr-v0/`), and a live Tesseract rendered-image `ocr-check` when `tesseract` is installed. The v0 corpus PDFs are committed under `test/v0/`, so these gates run in CI; only the root-level datasheet corpus remains non-committed local data.

Use `scripts/verify.sh --dry-run` to print the exact gate commands without running them. Set `GLYPHRUSH_VERIFY_PDFIUM=1` to include the PDFium-feature checks that protect the recommended LiteParse speed-race path: selected-backend feature parity, rendered-image OCR handoff, and the local `test/corpus.v0.json` quality gate when the ignored v0 PDFs are present. GitHub CI sets this flag so the PDFium path does not drift while the default local gate remains dependency-light.

Use `scripts/bench-liteparse.sh --dry-run` to inspect the repeatable LiteParse speed gate: it preflights the baseline wrappers, runs the quality-backed PDFium benchmark, and, when saving a report, verifies speed evidence through `feature-parity`. Saved-report runs emit external-baseline progress to stderr before each wrapper invocation and tee that progress to `<report>.progress.log`; override with `GLYPHRUSH_BENCH_PROGRESS_LOG=<path>`. The default script path filters the seed manifest to `datasheet` and smokes the target PDFs before benchmarking; set `GLYPHRUSH_BENCH_CATEGORY=all GLYPHRUSH_BENCH_MANIFEST=test/corpus.v0.json GLYPHRUSH_BENCH_COVERAGE_PRESET=glyphrush-v0` for the checked-in v0 mixed-category corpus, or `GLYPHRUSH_BENCH_CATEGORY=native-text GLYPHRUSH_BENCH_MANIFEST=test/corpus.v0.json` for the non-scanned native-text v0 speed gate. The native-text shortcut emits `--eval-category-preset glyphrush-v0-native-text` and defaults coverage to `glyphrush-v0-native-text`. That v0 manifest defaults the benchmark PDF root to `test/v0`, uses describe-only baseline preflight to avoid parsing the full external-baseline corpus twice, and raises the benchmark per-baseline timeout to 900000 ms for large fixtures; use `GLYPHRUSH_BENCH_PREFLIGHT=smoke`, `describe`, or `none`, plus `GLYPHRUSH_BENCH_PDF_DIR=<dir>` or `GLYPHRUSH_BENCH_BASELINE_TIMEOUT_MS=<ms>`, for custom preflight, corpus layout, and timeout policy. Set `GLYPHRUSH_BENCH_PROBE_PDF=<pdf>` to run only a bounded `baseline-check --pdf` probe for one suspected stalled document; add `GLYPHRUSH_BENCH_PROBE_BASELINE=liteparse` or another preset baseline name to isolate one wrapper instead of probing all four. Probe mode defaults to a 60000 ms baseline timeout and writes a baseline-check JSON report when `GLYPHRUSH_BENCH_OUTPUT=<path>` is set. For new full-corpus PDFs, put files under category folders such as `test/v0/clean_digital/`, `test/v0/scanned/`, or `test/v0/tables/`, then generate the manifest with `manifest test/v0 --category-from-path --coverage-preset glyphrush-v0`.
...
cargo run -p glyphrush-cli -- feature-parity --bench-report .glyphrush-baselines/reports/liteparse-v0-native-text.json --require-speed-advantage --require-coverage-preset glyphrush-v0-native-text
...
GLYPHRUSH_BENCH_CATEGORY=all GLYPHRUSH_BENCH_MANIFEST=test/corpus.v0.json GLYPHRUSH_BENCH_COVERAGE_PRESET=glyphrush-v0 GLYPHRUSH_BENCH_OUTPUT=.glyphrush-baselines/reports/liteparse-v0-gate.json scripts/bench-liteparse.sh
GLYPHRUSH_BENCH_CATEGORY=native-text GLYPHRUSH_BENCH_MANIFEST=test/corpus.v0.json GLYPHRUSH_BENCH_OUTPUT=.glyphrush-baselines/reports/liteparse-v0-native-text-gate.json scripts/bench-liteparse.sh
GLYPHRUSH_BENCH_MANIFEST=test/corpus.v0.json GLYPHRUSH_BENCH_PROBE_PDF=test/v0/academic_columns/acl-bert-naacl-2019.pdf GLYPHRUSH_BENCH_OUTPUT=.glyphrush-baselines/reports/liteparse-v0-academic-probe.json scripts/bench-liteparse.sh
GLYPHRUSH_BENCH_MANIFEST=test/corpus.v0.json GLYPHRUSH_BENCH_PROBE_PDF=test/v0/academic_columns/acl-bert-naacl-2019.pdf GLYPHRUSH_BENCH_PROBE_BASELINE=liteparse GLYPHRUSH_BENCH_OUTPUT=.glyphrush-baselines/reports/liteparse-v0-academic-liteparse-probe.json scripts/bench-liteparse.sh
...
The global `--backend` option defaults to `auto`, which resolves to the fastest enabled backend for the current binary. Plain builds resolve `auto` to `lopdf`; builds compiled with `--features pdfium` resolve `auto` to the PDFium adapter, so the faster native backend is the default when it is available. You can still select `--backend lopdf` explicitly for dependency-light runs, and `--backend pdfium` explicitly in PDFium builds. The optional adapter uses `pdfium-auto`, which may download and cache a matching PDFium runtime the first time the PDFium backend is actually used.

`backend-check` emits `glyphrush-backend-check-report-v1` with parser version, selected backend, enabled backend count, candidate backend count, and per-backend capability/limitation metadata. Add `backend-check --pdf <file-or-directory>` to smoke the selected backend against one PDF or every recursively discovered PDF in a directory and report open/extract success, page counts, native text bytes, image artifact count, OCR-required pages, source size/fingerprints for files, wall time, stable failure `error_kind` values such as `encrypted_pdf_requires_password`, sorted per-document results for directories, and bounded directory `failure_samples` so a mixed corpus failure is visible without scanning every document. Use `--jobs <N>` with directory smoke tests to run PDFs concurrently while merging results back into stable relative-path order and reporting the effective worker count. The PDFium adapter reuses the document opened for page counting during extraction, but caps corpus-level document workers to one so live PDFium document handles are not extracted concurrently in one process; reports expose this as `worker_count: 1`. Default builds report `lopdf` as enabled and PDFium/MuPDF as `not_wired`; PDFium-feature builds report `lopdf` and `pdfium` as enabled while MuPDF remains `not_wired`. PDFium-feature reports set `render_pages: true` because the adapter can render page images for OCR handoff, while `builtin_ocr` remains false.

`feature-parity` emits `glyphrush-feature-parity-report-v1`, a conservative LiteParse comparison matrix that separates implemented, partial, planned, and intentionally-not-planned capabilities; its `readiness` block distinguishes native-text speed-race capability readiness from an evidence-backed faster-than-LiteParse claim, and keeps built-in OCR, Python/Node wrappers, WASM, layout/table maturity, and the quality-backed speedup gate visible instead of relying on README prose. `native_text_speed_claim_ready` is false until a saved benchmark report supplies passing LiteParse and LiteParse-no-OCR speedup claims plus the recommended native-text coverage preset, and `native_text_speed_claim_blockers` explains the missing evidence. `native_text_speed_advantage_ready` is a separate, narrower signal: it can be true when Glyphrush quality passed, speed thresholds passed, baseline quality was checked, and coverage passed, even if LiteParse itself failed the quality labels; that keeps "Glyphrush is faster with passing quality" distinct from the stricter "both parsers passed quality, and Glyphrush is faster" claim. Add `--require-speed-advantage` when that narrower claim should be a hard gate; keep `--require-speed-evidence` for the stricter LiteParse-quality-backed claim. When `feature-parity --bench-report <saved-bench-json>` is used, `benchmark_evidence.quality_categories` lists the eval categories, document counts, page counts, failed checks, and pass state behind the speed evidence; `benchmark_evidence.coverage_requirement` also lists the requested target, present and missing categories, and whether that target was required for the run. The parity check revalidates that required LiteParse claims are quality-backed, timing-comparable, speed-passing, and backed by actual measured speedups at or above the required threshold; each embedded claim preserves the saved report's `glyphrush_quality_*`, `baseline_quality_*`, `glyphrush_quality_backed`, and `quality_blocker` fields so blocked claims show whether the gap is timing, Glyphrush quality, unchecked baseline quality, or failed baseline quality. `benchmark_evidence.baseline_quality_failures` summarizes failed baseline-quality documents by baseline and category, plus bounded failed-document samples, so a blocked LiteParse claim can be diagnosed without reopening the full benchmark report. `benchmark_evidence.baseline_quality_unchecked_categories` derives category-level unchecked baseline quality from saved report document details, including older reports that predate the direct corpus summary fields, so a blocked v0 report can show whether unscored baseline quality came from scanned/OCR-only docs, timeouts, or execution failures. If a long saved benchmark is interrupted and leaves an unreadable or zero-byte report, `benchmark_evidence.report_valid` is false, `benchmark_evidence.report_error` records the read/decode failure, and readiness reports `invalid_benchmark_report` instead of failing before emitting JSON. Use `--eval-category-preset glyphrush-v0-native-text --require-coverage-preset glyphrush-v0-native-text` for the native-text speed claim, or `--require-coverage-preset glyphrush-v0` when a full v0 run must also include the dedicated scanned/OCR category.
...
Reflow-profile datasheet tables with Sn-Pb/Pb-free assembly columns are recovered when routed for table recovery, including PDFium value groups that arrive below their feature labels.

Classification-temperature datasheet tables with package-thickness and volume columns are also recovered when routed for table recovery, with captions kept outside the structured table payload.

AWINIC-style electrical tables with `Parameter / Test Condition / Min. / Typ. / Max. / Unit` headers can route table recovery from the native text header alone and normalize wrapped labels, inherited conditions, split frequency-range conditions, symbol-font micro units, split `ppm/° C` units, ohm rows, thermal shutdown temperature rows, and unit-only continuation lines into structured cells while keeping page footers outside the table.

Datasheet electrical tables with `Parameter / Symbol / Conditions / Min. / Typ. / Max. / Unit` headers can also route table recovery from the native text header alone and normalize wrapped labels, inherited condition continuations, unit-only continuations, symbol-font delta/micro units, thermal shutdown rows, and EN threshold tail rows into structured cells while keeping notes and footers outside the table.

When comparing backends, prefer labeled text/table recall and silent-failure checks over route-count parity alone. Manifests generated from one backend can encode that backend's fallback behavior; for example, a page that `lopdf` marked `requires_ocr` because it extracted no native text may not require OCR under PDFium if PDFium extracts usable native text. Use per-document `expect_by_backend` overrides for backend-specific diagnostics such as route counts, OCR-required page counts, image artifact counts, or page flags while keeping shared labeled quality checks in `expect`. Eval and embedded bench-quality reports shallow-merge `expect_by_backend.<backend>` over `expect` for the artifact backend being scored; baseline text quality checks use only the shared `expect` fields. Use `baseline_required_text` inside `expect` for OCR-capable text baselines when Glyphrush should pass by explicitly flagging a no-OCR page as `requires_ocr` instead of emitting OCR text itself.

Local PDFs can be dropped into `test/`. PDF files in that directory are ignored by git so benchmark corpora do not get committed accidentally. File and directory `inspect` outputs include parser/backend/source metadata. `inspect --pages <pdf-or-directory>` runs the normal extraction/classifier path and emits compact page triage summaries: page artifact IDs, page fingerprints, dimensions, per-stage timings, route, quality flags, route reasons, native/OCR span counts, native text bytes, image artifact counts, bounded layout summaries, recovered table row/cell counts, and page warnings. Use `inspect --pages --jobs <N>` to triage a single PDF with page workers or a directory with per-PDF workers; results are merged back into stable page or relative-path order and the effective worker count is reported as `worker_count`. Use `inspect --pages --cache-dir <dir>` for warm repeated triage; single-file output reports `cache_status`/`cache_key`, while directory output also reports aggregate `cache_hits` and `cache_misses` plus per-document cache status. Directory `inspect --pages`, `inspect`, and `bench` commands recursively discover `.pdf`/`.PDF` files in stable relative-path order and emit corpus-level aggregate JSON with a `corpus_fingerprint` over the ordered document labels, document fingerprints, and page counts.
...
Layout reconstruction v0 is text-derived. Glyphrush splits native or OCR text into deterministic page-local blocks, reflows common short extraction fragments, and classifies simple headings, lists, tables, and paragraphs. When opt-in span geometry produces usable boxes, the layout path can preserve full-width title/banner bands, fragmented full-width heading rows, leading, middle, and trailing cross-column bands, conservative short section-heading separators, column-row bands that keep centered banners, gutter-straddling rows, and trailing page numbers out of column splits, and clearly separated 2-5 column reading order before falling back to vertical-gap grouping. Each page artifact records the selected reading-order strategy as `layout_strategy` (for example `column_row_bands`, `full_width_bands`, or `vertical_gaps`), and `debug-page` includes it so layout escalation decisions are inspectable. If a page shows strong multi-column evidence that no column strategy could resolve, the page is flagged `layout_uncertain` with a `column_layout_unresolved` reason instead of silently interleaving column text. `test/corpus.v0.layout.json` is the labeled span-geometry layout gate for the local v0 corpus: it pins reading-order sequences, span-bbox samples, per-page layout-block counts, and hand-labeled ruled-table structure for multi-column, hybrid-scan, landscape-form, and invoice-voucher fixtures; run it with `eval test/corpus.v0.layout.json --span-geometry`. The PDFium backend also extracts positioned vector ruling lines (composed through nested form XObject transforms) on table-routed pages; when span geometry is enabled, column-ruled grids are recovered from those rulings with text-row row structure, blank-cell preservation, and diagram-lattice rejection, which is how the filled GSA SF-1035 invoice fixture (`tools/baselines/make_invoice_fixture.py`) and the IRS f1095-C month grid produce structured cells. Repeated positioned blocks in the top or bottom page margin are classified as `header` or `footer` when the same normalized text appears on multiple pages. The page classifier also treats dense stroked horizontal/vertical ruling lines as `table_uncertain` so ruled tables are escalated even when extracted text has no pipe/tab delimiters. When that table route is active, simple consistent whitespace rows are preserved and exposed as table blocks for quality gates; fixed-width whitespace rows can also preserve blank cells by aligning row segments to header column starts, merge lowercase wrapped descriptor fragments into the following row, and keep section rows inside the grid; key-value metadata rows are recovered as conservative `Field` / `Value` tables; embedded pin/function, number-first pin-description, package pin-description, part-number ordering, and OMB-style budget projection table patterns split common grids into structured cells; pipe/tab and header-guided whitespace rows can leave leading captions, including captions after surrounding prose, as surrounding text blocks, and header-guided rows can also merge same-line or wrapped leading multi-word descriptor cells against short header columns, merge lowercase trailing descriptor continuations into the preceding row, keep inferred trailing blank cells explicit, and keep title-case section rows inside the grid; if positioned spans form aligned multi-column rows, Glyphrush groups them as a table before applying the geometry reading-order heuristic, keeps omitted cells explicit when rows skip blank columns, merges same-line fragmented positioned cells, same-column wrapped header rows, plus near-adjacent single- or multi-cell wrapped positioned continuations into the same structured row, keeps cross-column, first-column, or fragmented first-column interior section rows inside the table grid with blank cells for the remaining columns, and leaves top/bottom captions as surrounding text blocks. Table blocks include structured `table.rows[].cells[]` in JSON artifacts; pipe/tab, fixed-width whitespace, key-value metadata, datasheet pin/ordering, budget projection, and positioned-row tables preserve empty cells, positioned cells with source spans include bounding boxes when span geometry is available, and text-only or omitted cells omit cell boxes. Plain text, markdown, and eval quality text prefer layout-block order when blocks are available while raw `native_spans` remain preserved in JSON artifacts. Markdown export normalizes structured, pipe-delimited, or whitespace-delimited table blocks into markdown tables with a generated separator row; other table blocks remain plain text.

Fragmented datasheet electrical tables such as `Symbol / Parameter / Rating / Unit` are also recovered when table routing is active, including PDFium extraction shapes that insert blank lines between symbol and value fragments.

Bullet/leader datasheet rows such as absolute-maximum rating lists are normalized into two-column `Parameter` / `Limit` tables when repeated leader separators make the row structure explicit.

Electrical-characteristics datasheet tables with `Symbol / Parameter / Test Conditions / Min. / Typ. / Max. / Unit` headers are recovered when routed for table recovery, including blank symbol cells, unit-only continuation lines, and PDFium row-order splits around dropout-voltage conditions.

Native span geometry is currently conservative and opt-in. Add `--span-geometry` to `parse`, `bench`, `debug-page`, or `eval` when you want bounded/simple `lopdf` text-positioning streams or PDFium text segments to produce multiple boxed spans, plus a measured `bbox_overlap_ratio` layout-risk signal. The `lopdf` path approximates boxes with simple text-matrix and content-matrix transforms plus text-state parameters preserved across text objects, `TJ`/`Tc`/`Tw`/`Tz` spacing, `Ts` text rise, `TL` leading, and `'`/`"` text-showing shortcut adjustments applied. The PDFium path uses PDFium's merged text-segment rectangles, converts them into Glyphrush page-local coordinates, and uses a higher bounded native-text cap because it does not need Glyphrush to walk the PDF content stream. Geometry-aware layout handles narrow academic gutters with trailing centered page numbers as separate body-column and margin groups. The default hot path emits page-wide native spans. Large `lopdf` streams, very large PDFium text pages, large native text on content-stream positioning, or unsupported geometry such as rotation fall back to the page-wide native span and are flagged and warned with `unsupported_feature` plus `span_geometry_capped` instead of silently pretending requested bbox detail was produced. Decoded spans that do not match the backend's native text output still fall back to the page-wide native span instead of taxing the hot path or risking bad geometry.
...
`manifest <pdf-or-directory>` emits an eval-compatible JSON skeleton for the current parser output. It is useful after dropping new PDFs into `test/`: save the output next to the PDFs, run `eval` to confirm the structural gates, then review any generated `table_structure` and, when `--span-geometry` is enabled, bounded `span_bbox` checks, and tighten the manifest with human/labeled document-level `required_text`, `text_recall`, `reading_order`, `table_structure`, or additional `span_bbox` expectations. Eval `required_text` uses layout-block order, checks exact substrings first, then falls back to whitespace-normalized matching so layout-only line wrapping does not fail content anchors. It serializes structured table grids as pipe-delimited rows, preserving blank cells such as `| A |  | missing value |` for table-quality anchors. Use `manifest --category <name>` to stamp every generated document with a benchmark coverage class such as `datasheet`, `clean_digital`, or `scanned`; downstream eval and benchmark reports then populate category counts and category summaries without hand-editing the generated JSON. Use `manifest --category-from-path` for mixed corpora laid out as `test/<category>/<file>.pdf`; manifest generation recursively discovers those PDFs, stores relative paths such as `clean_digital/example.pdf`, and uses the top-level folder as each document category. Use `manifest --coverage-preset glyphrush-v0` to add the core v0 corpus coverage gate for `clean_digital`, `scanned`, `hybrid`, `academic_columns`, `tables`, `forms`, `rotated`, `weird_encoding`, and `large`, with each category required at least once. Use `manifest --coverage-preset glyphrush-v0-native-text` for the same v0 surface without the dedicated scanned category when building native-text/no-OCR speed evidence. Use repeated `manifest --required-category <name>` to generate the top-level presence gate when a corpus must include specific classes before speed/quality claims are accepted, and repeated `manifest --min-category-count <name=count>` to require enough documents in a class before accepting a corpus benchmark. Use `manifest --jobs <N>` to generate directory manifests with parallel per-PDF workers or single-PDF manifests with page workers; results are merged back into deterministic relative-path/page order and the generated JSON does not include worker-count provenance, so corpus fingerprints and expectations remain stable across worker counts. Use `manifest --cache-dir <dir>` to reuse parsed artifacts across repeated manifest generation; cache status is intentionally not serialized into the manifest so cold and warm generated JSON remain comparable. The generated skeleton includes deterministic generator provenance, a corpus fingerprint, optional `required_categories`, optional `min_category_counts`, optional per-document category, per-document source fingerprints, sizes, and modified times checked by `eval`, page counts, route counts, quality-flag counts, OCR-needed classification, non-empty quality-flag classification gates, warning counts, exact required warnings, image artifact counts, recovered table-structure checks for pages with table blocks, bounded span-bbox checks for non-page-wide spans when span geometry is requested, `silent_failures: 0`, and per-page artifact ID, page fingerprint, route, bounded page-local required-text anchor, layout-block, image, flag, and reason acknowledgements so current fallback decisions and diagnostics are explicit instead of implicit.
