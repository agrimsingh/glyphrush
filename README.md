...
```js
import { bench, evalManifest, inspectPages, parse, parseText } from "./bindings/node/src/index.mjs";
...
The Python and Node wrappers expose `parse`, text output, `inspect --pages` triage, `eval` quality reports, and `bench` speed reports while remaining dependency-free shims over the native binary. They do not implement independent PDF parsing, OCR, layout, quality scoring, benchmarking, or cache logic; wrapper calls pass backend, cache, jobs, span-geometry, category, baseline, speed-claim, and OCR adapter options through to the CLI and decode the same JSON artifacts.
