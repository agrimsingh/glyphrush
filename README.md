...
`scripts/verify.sh` is the shared local/GitHub CI gate. It runs formatting, Python and Node wrapper tests, the full Rust workspace test suite, clippy with warnings denied, strict `glyphrush-v0` baseline-preset metadata preflight, the datasheet eval gate when ignored root-level local PDFs exist under `test/`, and the PDFium-generated v0 eval gate when ignored local PDFs exist under `test/v0/` and `GLYPHRUSH_VERIFY_PDFIUM=1` is set. In a fresh GitHub checkout those PDFs are absent by design, so CI skips only the local corpus gates rather than failing on non-committed benchmark files.
