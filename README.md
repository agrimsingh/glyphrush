...
Use `scripts/bench-liteparse.sh --dry-run` to inspect the repeatable LiteParse speed gate: it preflights the baseline wrappers against the target PDFs, runs the quality-backed PDFium benchmark, and, when saving a report, verifies speed evidence through `feature-parity`. The default script path filters the seed manifest to `datasheet`; set `GLYPHRUSH_BENCH_CATEGORY=all GLYPHRUSH_BENCH_MANIFEST=test/corpus.v0.json GLYPHRUSH_BENCH_COVERAGE_PRESET=glyphrush-v0` for the checked-in v0 mixed-category corpus. That v0 manifest defaults the benchmark PDF root to `test/v0` and raises the per-baseline timeout to 900000 ms for large fixtures; use `GLYPHRUSH_BENCH_PDF_DIR=<dir>` or `GLYPHRUSH_BENCH_BASELINE_TIMEOUT_MS=<ms>` for custom corpus layouts and timeout policy. For new full-corpus PDFs, put files under category folders such as `test/v0/clean_digital/`, `test/v0/scanned/`, or `test/v0/tables/`, then generate the manifest with `manifest test/v0 --category-from-path --coverage-preset glyphrush-v0`.
