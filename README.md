...
```js
import { inspectPages, parse, parseText } from "./bindings/node/src/index.mjs";
...
The Python and Node wrappers expose `parse`, text output, and `inspect --pages` triage while remaining dependency-free shims over the native binary. They do not implement independent PDF parsing, OCR, layout, or cache logic; wrapper calls pass backend, cache, jobs, span-geometry, and OCR adapter options through to the CLI and decode the same JSON artifacts.
