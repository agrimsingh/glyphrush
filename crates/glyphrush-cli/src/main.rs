...
fn liteparse_feature_parity_capabilities(
    ...
    vec![
        ...
        FeatureParityCapability {
            ...
            notes: "Glyphrush avoids always-on per-character metadata, preserves full-width bands, leading, middle, and trailing cross-column bands, and conservative short section separators around clearly separated two-column sections when span geometry is available, seeds bounded span-bbox manifest samples, and escalates layout work when signals require it.",
        },
        FeatureParityCapability {
            id: "ocr",
            area: "ocr",
        ...
        FeatureParityCapability {
            ...
            notes: "Current table support is conservative, tied to explicit uncertainty flags, preserves blank cells for delimited text, fixed-width whitespace, fixed-width wrapped descriptor fragments, header-guided whitespace rows with same-line or wrapped multi-word descriptor cells, trailing descriptor continuations, header-guided trailing blank cells, and header-guided section rows, aligned whitespace and positioned section rows, and aligned positioned rows including same-line fragmented positioned cells, first-column positioned section rows, fragmented first-column positioned section rows, and multi-cell wrapped continuations when table recovery is routed, and exposes structured grids to eval text anchors.",
        },
        FeatureParityCapability {
