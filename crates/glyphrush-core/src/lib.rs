...
#[derive(Clone, Debug)]
struct RulingCluster {
    position: f32,
    start: f32,
    end: f32,
}

fn cluster_ruling_lines(lines: &[ExtractedRulingLine], tolerance: f32) -> Vec<RulingCluster> {
    if lines.is_empty() {
        return Vec::new();
    }
...
fn cluster_y_overlap_fraction(cluster: &RulingCluster, band_start: f32, band_end: f32) -> f32 {
...
fn span_center_x(span: &TextSpan) -> f32 {
    (span.bbox.x0 + span.bbox.x1) / 2.0
}

fn grid_interval_index(value: f32, positions: &[f32]) -> Option<usize> {
    (0..positions.len().saturating_sub(1))
        .find(|&index| value > positions[index] && value < positions[index + 1])
}

fn span_straddles_vertical_boundary(span: &TextSpan, vertical_positions: &[f32]) -> bool {
    vertical_positions
        .iter()
        .any(|&position| span.bbox.x0 + 4.0 < position && position < span.bbox.x1 - 4.0)
}

fn ruled_grid_row_has_non_overlapping_columns(row_cells: &[Vec<&TextSpan>]) -> bool {
...
fn ruled_grid_cell_text(spans: &[&TextSpan]) -> String {
...
fn looks_like_ruled_grid_wrapped_continuation_row(
...
fn merge_wrapped_ruled_grid_table_rows(rows: Vec<Vec<Vec<&TextSpan>>>) -> Vec<Vec<Vec<&TextSpan>>> {
...
fn is_fragmented_layout_band_row(row: &[&TextSpan], dimensions: &PageDimensions) -> bool {
...
fn is_fragmented_column_section_separator_row(
    row: &[&TextSpan],
    dimensions: &PageDimensions,
) -> bool {
    if !(2..=4).contains(&row.len()) || !fragmented_row_spans_are_tightly_joined(row) {
...
fn package_pin_description_table_rows(lines: &[&str]) -> Option<Vec<Vec<String>>> {
    let (rows, consumed) = package_pin_description_table_rows_prefix(lines)?;
    (consumed == lines.len()).then_some(rows)
}

fn package_pin_description_table_rows_prefix(lines: &[&str]) -> Option<(Vec<Vec<String>>, usize)> {
...
fn is_package_pin_description_table_start(line: &str) -> bool {
    normalize_pin_table_header(line) == "pin number"
}

fn package_pin_description_headers(lines: &[&str]) -> Option<(Vec<String>, usize)> {
...
fn package_pin_description_data_row(
    tokens: &[&str],
...
fn looks_like_package_pin_cell(token: &str) -> bool {
    let token = token.trim_matches(|ch: char| matches!(ch, ',' | ';'));
    is_dash_placeholder(token) || token.chars().all(|ch| ch.is_ascii_digit())
}

fn looks_like_pin_name_or_placeholder(token: &str) -> bool {
    let token = token.trim_matches(|ch: char| matches!(ch, ',' | ';'));
...
fn normalize_package_pin_cell(token: &str) -> String {
    token
        .trim_matches(|ch: char| matches!(ch, ',' | ';'))
        .to_string()
}
